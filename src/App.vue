...
//Add/Remove city from the hotbar, save results to local storage
...
//Load data from local storage and load weather data for first entry
onMounted(async () => {
...
<template>
    <div class="bg-gradient-to-b from-sky-600 from-10% via-sky-500 via-30% to-amber-200 min-h-screen flex flex-col">
        <Header :activeCityName="activeCityName" :searching="searching" @toggleSearch="toggleSearch" />
        <div v-if="!searching" class="flex-grow">
            <PinnedCities :cities="pinnedCities" :activeCityId="activeCityId" @citySelected="updateActiveCity" />
            <Weather v-if="pinnedCities.length > 0 || activeCityName !== 'Empty'" :activeCityName="activeCityName"
                :pinnedCities="pinnedCities" :metric="measurementUnits" @togglePinned="togglePinned"
